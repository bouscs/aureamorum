 ...
 */
export type ObjectPath<O, Path extends string> = Path extends `${infer K}.${infer Rest}` ? K extends keyof O ? ObjectPath<O[K], Rest> : never : Path extends keyof O ? O[Path] : never;
export type TupleToSum<T extends unknown[]> = T extends [infer L, ...infer R] ? L extends unknown ? R extends unknown[] ? L | TupleToSum<R> : L : unknown : unknown;
